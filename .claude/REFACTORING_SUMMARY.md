...
#### 新增的参数
- `--reset-day` (u8, 默认 1) - 每月重置日期（1-31，超过当月天数时使用最后一天）
...
- 当前日期 ≥ 有效重置日期（`get_effective_reset_day(reset_day)`）

**月末智能处理**:
- 如果 `reset_day` 超过当月天数，使用当月最后一天
- 例如：`reset_day=31` 时
  - 1/3/5/7/8/10/12 月：在 31 号重置
  - 4/6/9/11 月：在 30 号重置
  - 2 月：平年在 28 号，闰年在 29 号重置
...
1. **reset_day 范围为 1-31，自动适应月末**
   - 支持设置 29/30/31 号
   - 当月天数不足时自动使用最后一天
   - 例如设置 31 号，2 月会在 28/29 号重置
