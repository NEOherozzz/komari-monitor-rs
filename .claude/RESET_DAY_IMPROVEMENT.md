...
**重置时机说明**:
- 重置发生在 `reset_day` **当天**（00:00 - 23:59 之间的任意时刻）
- 程序每隔 `network_interval` 秒检查一次（默认 10 秒）
- 当 `current_day >= effective_reset_day` 且月份改变时，触发重置
- **关键**：`>=` 运算符意味着在 reset_day 当天或之后首次检查时触发
- 例如：reset_day=5，则在 5 号 00:00 之后的第一次检查时重置
...
## reset_day 和流量重置时间的关系

### 核心概念

**重要**：流量重置发生在 `reset_day` **当天**，而不是前一天或后一天。

### 时间关系说明

```
reset_day 设置 → 决定重置日期 → 重置在该日期当天发生
```

**具体时间点**：
- 重置日期：`reset_day` 指定的日期（经过月末智能调整后）
- 重置时刻：该日期的 00:00 - 23:59 之间的任意时刻
- 检查频率：每 `network_interval` 秒检查一次（默认 10 秒）
...
| 日期 | current_month | last_reset_month | current_day | effective_reset_day | 是否重置 | 说明 |
|------|---------------|------------------|-------------|---------------------|----------|------|
| 1月4日 | 1 | 1 | 4 | 5 | ❌ | 同月且未到5号 |
| 1月5日 00:00+ | 1 | 1 | 5 | 5 | ❌ | 同月（首次运行会设置last_reset_month=1） |
| 2月4日 | 2 | 1 | 4 | 5 | ❌ | 月份已变但未到5号 |
| 2月5日 00:00+ | 2 | 1 | 5 | 5 | ✅ | 月份已变且到达5号，**触发重置** |
| 2月6日 | 2 | 2 | 6 | 5 | ❌ | 同月（已在5号重置过） |

### 关键要点

1. **重置日 = 流量归零日**
   - `reset_day=1`：每月 1 号归零
   - `reset_day=15`：每月 15 号归零
   - `reset_day=31`：每月最后一天归零

2. **不会提前或延后**
   - ✅ 重置发生在 reset_day 当天
   - ❌ 不会在 reset_day-1 或 reset_day+1 重置

3. **精确时刻取决于检查周期**
   - 默认每 10 秒检查一次
   - 重置发生在该天首次检查时
   - 通常在 00:00:00 - 00:00:10 之间（假设程序持续运行）

4. **延迟启动处理**
   - 如果程序在重置日之后启动，会立即触发重置
   - 例如：reset_day=5，程序在 2月10日 启动，会立即重置
...
- 所有月份都在 1 号重置
- 固定行为，无月末适配
- **重置时间**：每月 1 号的 00:00:00 - 00:00:10 之间（假设程序持续运行）
...
- 所有月份都在 15 号重置
- 固定行为，无月末适配
- **重置时间**：每月 15 号的 00:00:00 - 00:00:10 之间
- **适用场景**：VPS 账单周期从每月 15 号开始
...
| 月份 | 天数 | 实际重置日期 | 重置时间范围 |
|------|------|--------------|--------------|
| 1 月 | 31 | 31 号 | 31号 00:00-00:10 |
| 2 月（平年）| 28 | **28 号** | 28号 00:00-00:10 |
| 2 月（闰年）| 29 | **29 号** | 29号 00:00-00:10 |
| 3 月 | 31 | 31 号 | 31号 00:00-00:10 |
| 4 月 | 30 | **30 号** | 30号 00:00-00:10 |
| 5 月 | 31 | 31 号 | 31号 00:00-00:10 |
| 6 月 | 30 | **30 号** | 30号 00:00-00:10 |
| 7 月 | 31 | 31 号 | 31号 00:00-00:10 |
| 8 月 | 31 | 31 号 | 31号 00:00-00:10 |
| 9 月 | 30 | **30 号** | 30号 00:00-00:10 |
| 10 月 | 31 | 31 号 | 31号 00:00-00:10 |
| 11 月 | 30 | **30 号** | 30号 00:00-00:10 |
| 12 月 | 31 | 31 号 | 31号 00:00-00:10 |
...
| 月份 | 天数 | 实际重置日期 | 重置时间范围 |
|------|------|--------------|--------------|
| 1 月 | 31 | 30 号 | 30号 00:00-00:10 |
| 2 月（平年）| 28 | **28 号** | 28号 00:00-00:10 |
| 2 月（闰年）| 29 | **29 号** | 29号 00:00-00:10 |
| 其他月 | 30/31 | 30 号 | 30号 00:00-00:10 |
