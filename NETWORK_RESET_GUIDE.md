...
**重置时间说明**：
- **重置发生在 `reset_day` 当天**（从 00:00 到 23:59 之间的任意时刻）
- 具体重置时刻取决于程序检查时间（每 `network_interval` 秒检查一次，默认 10 秒）
- 重置后，该月的流量统计从 0 开始重新计算
- **不是在 reset_day 前一天或后一天重置，而是当天重置**

**配置方式**：
```bash
# 设置每月 5 号重置流量（会在 5 号当天的某个时刻重置）
...
**按月重置触发条件**：
程序每隔 `network_interval` 秒（默认 10 秒）检查一次，当同时满足以下两个条件时触发重置：
1. 当前月份 ≠ 上次重置月份（`last_reset_month`）
2. 当前日期 ≥ 设置的重置日期（`reset_day`）

**重置时机说明**：
- 重置发生在 `reset_day` **当天**（00:00 - 23:59 之间）
- 不会在 reset_day 之前或之后的日期重置
- 如果程序在重置日期之后才启动，会在启动时立即重置
- 如果当月没有配置的日期（如 2 月 31 日），使用当月最后一天

**重置时的操作**：
1. 记录当前系统流量作为新的 `source_tx/rx`（新的基准点）
2. 清零 `latest_tx/rx`（当前启动周期的累计流量）
3. 清零 `accumulated_tx/rx`（跨启动累计的流量）
4. 更新 `last_reset_month` 为当前月份
5. 立即保存到配置文件

**示例时间线**：
```
# 设置 reset_day=1（每月 1 号重置）
1月1日 00:00-23:59  - 当天某个时刻自动重置，source=当前系统流量，latest=0
1月2日 - 1月31日    - 累计流量到 latest，不会重置
2月1日 00:00-23:59  - 当天某个时刻自动重置，source=当前系统流量，latest=0（重新开始）

# 设置 reset_day=5（每月 5 号重置）
1月4日 23:59  - 还未到重置日期，不重置
1月5日 00:00+ - 到达重置日期，当天某个时刻自动重置
1月6日+      - 已过重置日期，本月不再重置

# 设置 reset_day=31（每月最后一天重置）
1月31日 - 自动重置（1月有31天，在31日重置）
2月28日 - 自动重置（2月平年只有28天，自动使用最后一天）
2月29日 - 自动重置（2月闰年有29天，自动使用最后一天）
3月31日 - 自动重置（3月有31天，在31日重置）
4月30日 - 自动重置（4月只有30天，自动使用最后一天）
```

**关键点**：
- 重置日 = 流量归零日（在这一天的某个时刻重置）
- reset_day=1 表示每月 1 号重置，不是 2 号，也不是月末
- reset_day=15 表示每月 15 号重置，适合账单周期从 15 号开始的场景
